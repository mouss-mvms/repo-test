...
RSpec.describe Api::V1::ShopsController, type: :controller do
  describe "GET #index" do
    context "All ok" do
      it 'should return some shops' do
        city = create(:city, slug: 'bordeaux')
        Shop.destroy_all
        times = 4
        times.times do
          create(:shop, city_id: city.id)
        end
        ...
        allow(::Requests::ShopSearches).to receive(:search_highest_scored_shops).and_return(mocks_searchkick)
        allow(::Requests::ShopSearches).to receive(:search_random_shops).and_return(mocks_searchkick)
        get :index, params: { location: city.slug }

        expect(response).to have_http_status(:ok)
        result = JSON.parse(response.body)
        expect(result).to be_instance_of(Array)
        expect(result.count).to eq(times*2)
      end

      context "with Fields filtered" do
        it 'should return some shops with fields asked and filtered' do
          city = create(:city, slug: 'bordeaux')
          Shop.destroy_all
          times = 4
          times.times do
          ...
          result.each do |r|
            ...
            expect(r).not_to have_key('description')
          end
        end
      end
    end
    ...
    context 'Params incorrect' do
      ...
      context 'Category not found' do
        it 'should return 404 HTTP status' do
          ...
          get :index, params: { location: city.slug, categories: category.slug }

          expect(response).to have_http_status(404)
          expect(response.body).to eq(Dto::Errors::NotFound.new("Category not found for slug #{category.slug}").to_h.to_json)
        end
      end
    end
  end
    ...
    context "All ok" do
      ...
      context "with Fields filtered" do
        it 'should return response with fields asked and filtered' do
          ...
          fields = %w[id name]
          get :shop_summaries, params: { location: city.slug, fields: fields }
          should respond_with(200)
          result = JSON.parse(response.body)
          expect(result).to be_instance_of(Array)
          result.each do |r|
